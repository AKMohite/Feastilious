...
//            add to shopping
      val ingredients =
        db.ingredientDAO().getIngredientsFor(recipeId).firstOrNull() ?: return@withContext
...
//            add to shopping
  ...
  override fun observeShoppingCartForRecipe(recipeId: Long): Flow<List<Shopping>> = db
    .shoppingDAO()
    .observeCartForRecipe(recipeId)
    .distinctUntilChanged()
    .flowOn(dispatcher.io)
    .map { entities ->
