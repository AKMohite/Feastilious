...
//            override val unconfined = testDispatcher
        ...
        repository = RealRecipesRepository(
            api = mockApi,
            db = mockDb,
    ...
    inner class RefreshRecipesTests {
        ...
        @Test
        fun `refreshRecipes when page greater than 5 should return without fetching`() =
        ...
        @Test
        fun `refreshRecipes when not forceRefresh and local data is valid should return without fetching`() =
            runTest(testDispatcher) {
                val requestType = SyncType.POPULAR_RECIPES
                val page = 1
                val currentTime = Clock.System.now().plus(1.hours)
                val validLastSync = LastSyncEntity(
                    id = 1L,
        ...
        @Test
        fun `refreshRecipes when data is stale should fetch from API and save to DB`() =
            runTest(testDispatcher) {
                val requestType = SyncType.POPULAR_RECIPES
                val page = 1
                ...
                val mockApiResponse = ComplexSearchDTO(
...
//            ) }
        ...
        @Test
        fun `refreshRecipes on forceRefresh should fetch from API even if local data is valid`() =
            runTest(testDispatcher) {
        ...
        @Test
        fun `refreshRecipes when API returns empty list should not save anything`() =
            runTest(testDispatcher) {
    ...
    inner class GetRecipesTests {
        @Test
        fun `getRecipes for POPULAR_RECIPES should call correct DAO and map results`() =
            runTest(testDispatcher) {
                val page = 1
                val mockRecipeEntities = listOf(
                    createRecipeFor(1),
                    createRecipeFor(2)
                )
                val expectedRecipes = recipesMapper.entitiesToModels(mockRecipeEntities)

                coEvery { mockDb.popularRecipeDAO().getRecipes(page) } returns flowOf(
                    mockRecipeEntities
                )
...
//                coVerify { mockDb.popularRecipeDAO().getRecipes(page) }
        ...
        fun `getRecipes for TOP_RATED_RECIPES should call correct DAO`() = runTest(testDispatcher) {
            ...
            repository.getRecipes(SyncType.TOP_RATED_RECIPES, page)
        ...
        fun `getRecipes for invalid SyncType should throw IllegalArgumentException`() {
            assertThrows(IllegalArgumentException::class.java) {
                runTest(testDispatcher) { // runTest needed if the function under test is suspend or uses coroutines
    ...
    inner class ObservePaginatedRecipesTests {
        ...
        @Test
        fun `observePaginatedRecipes should create Pager with correct PagingSourceFactory`() =
            runTest(testDispatcher) {
                val requestType = SyncType.POPULAR_RECIPES
                ...
                coEvery { mockLastSyncDao.getLastSync(requestType.name) } returns LastSyncEntity(
                    id = 1L,
                    entityType = requestType.name,
                ...
                coEvery { mockApi.searchRecipes(any()) } returns ComplexSearchDTO(
                    0,
                    0,
                    emptyList(),
                    0
        ...
        private fun mockIsLocallyAvailable(syncType: SyncType, page: Int, hasData: Boolean) {
