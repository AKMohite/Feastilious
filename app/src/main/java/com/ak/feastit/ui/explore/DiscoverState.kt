...
internal data class DiscoverState(
  ...
  private fun List<ExploreSection>.toAdapterItems(): List<ExploreAdapterItem> {
    ...
    sections.forEach { section ->
      when (section.row) {
        is ExploreRow.Chips -> mutableList.add(
          ExploreAdapterItem.HorizontalChips(
            section.category,
            section.row.contents,
          ),
        )

        is ExploreRow.RecipeRows -> mutableList.add(
          ExploreAdapterItem.HorizontalRecipes(
            section.category,
            section.row.contents,
            section.isLoading,
          ),
  ...
  fun isEmpty(): Boolean {
    return exploreSections().none { section ->
      ExploreCategory.staticCategoryElements()
        .any { it == section.category && section.row?.contents?.isEmpty() == true }.not()
    }
  }
...
internal data class ExploreSection(
  ...
  // TODO why we need to pass any *, need to check other
