...
internal enum class ExploreCategory {
  ...
  fun toSyncType(): SyncType = when (this) {
  ...
  fun isStaticCategory(): Boolean = staticCategoryElements().contains(this)

  companion object {
    fun fromName(name: String): ExploreCategory? = ExploreCategory.entries.firstOrNull { it.name == name }

    /**
     * These categories have to synced
     */
    fun getRefreshExploreEntries(): List<ExploreCategory> = entries.filterNot { category ->
     ...
     */
    fun staticCategoryElements() = listOf(BANNER_RECIPES, MEAL_TYPE_CHIPS, CUISINE_TYPE_CHIPS, DIET_TYPE_CHIPS)

    /**
     * These categories are observed for changes from api sync
     */
    fun observerExploreEntries(): List<ExploreCategory> = listOf(
      POPULAR_RECIPES,
      TOP_RATED_RECIPES,
      HEALTHY_RECIPES,
      QUICK_RECIPES,
