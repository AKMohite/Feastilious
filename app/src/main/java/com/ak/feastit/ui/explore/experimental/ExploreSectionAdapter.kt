...
internal class ExploreSectionAdapter(
  ...
  AsyncListDiffer.ListListener<ExploreAdapterItem> {
  ...
  override fun onCreateViewHolder(
    ...
    TOP_HORIZONTAL_BANNER -> {
      val binding = ComponetExploreHeaderBannerBinding.inflate(
...
//                TODO handle list for banners
    ...
    HORIZONTAL_CHIPS -> {
      val binding = ComponentExploreSectionChipsBinding.inflate(
        LayoutInflater.from(parent.context),
        parent,
        false,
      )
    ...
    HORIZONTAL_RECIPES -> {
      val binding = ComponentExploreSectionRecipeBinding.inflate(
        LayoutInflater.from(parent.context),
        parent,
        false,
      )
...
internal sealed interface ExploreItemAction {
  ...
  data class RecipeClick(
    ...
    fun sharedElementsVarArgs(): Array<Pair<View, String>> = sharedElements.toList().toTypedArray()
  }

  data class ChipClick(
    val chip: ExploreChip,
  ) : ExploreItemAction
