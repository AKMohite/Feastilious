    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType) {
            SUGGESTION_TYPE_HEADING -> {
                val binding = ComponentSearchSuggestionHeadingBinding.inflate(LayoutInflater.from(parent.context), parent, false)
                ComponentSearchSuggestionHeading(binding)
            }
            SUGGESTION_TYPE_TEXT -> {
                val binding = ComponentSearchSuggestionTextBinding.inflate(LayoutInflater.from(parent.context), parent, false)
