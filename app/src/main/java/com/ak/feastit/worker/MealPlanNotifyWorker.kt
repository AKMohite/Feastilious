...
//    }
  ...
  override suspend fun doWork(): Result {
    ...
    val notificationTime = inputData.getString(WORK_NOTIFICATION_TIME)
      ?: throw IllegalArgumentException("No notification time found")
...
//        FIXME: notification title and message is too long so handled with String.take()
    val title =
      applicationContext.getString(R.string.notification_meal_plan_title, mealPlan.name.take(30))
    val message = applicationContext.getString(
      R.string.notification_meal_plan_message,
      mealPlan.name.take(15),
      notifyAt.time.toString(),
    )
    val notification =
      mealPlan.toNotification(title = title, message = message, notifyAt = notifyAt.toInstant())
